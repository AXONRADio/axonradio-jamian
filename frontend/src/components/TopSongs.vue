  ...
  <div>
    ...
    <v-layout mt-5>
      <v-flex offset-xs2 offset-sm1 offset-md1 offset-lg1 sm8 md6 lg4 xl1>
        ...
        <v-btn block @click="setVidId(popid)" color="pink lighten-2" light>Pop: {{pop.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(reggaeid)" color="purple lighten-3" light>Reggae: {{reggae.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(hiphopid)" color="purple lighten-2" light>HipHop: {{hiphop.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(discoid)" color="deep-purple lighten-3" light>Disco: {{disco.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(countryid)" color="deep-purple lighten-2" light>Country: {{country.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(classicalid)" color="indigo lighten-3" light>Classical: {{classical.slice(0,30)}}</v-btn>
        <v-btn block @click="setVidId(metalid)" color="indigo lighten-2" light>Metal: {{metal.slice(0,30)}}</v-btn>
      </v-flex>
...
export default {
  ...
  methods:{
    getTopData(){
      axios.get('http://167.99.98.179/api/top/')
      .then(response => {
